...
pub fn scan_projects_from_path(base_path: &str) -> Result<Vec<ClaudeProject>, String> {
    ...
    for entry in fs::read_dir(&tmp_dir).map_err(|e| format!("Failed to read tmp dir: {e}"))? {
        ...
        projects.push(ClaudeProject {
            ...
            path: format!("ditcodeagent://{}", project_dir.to_string_lossy()),
            actual_path,
            session_count,
            message_count,
            last_modified,
            git_info: None,
            provider: Some("ditcodeagent".to_string()),
...
pub fn load_sessions(
    ...
    // Validate project path is inside DITCodeAgent data directory
    let project_dir = validate_ditcodeagent_path(dir)?;
    let chats_dir = project_dir.join("chats");
...
/// Resolve and validate that a path is inside `~/.ditcodeagent/tmp/`
fn validate_ditcodeagent_path(raw_path: &str) -> Result<PathBuf, String> {
    let path = PathBuf::from(raw_path)
        .canonicalize()
        .map_err(|e| format!("Invalid path: {e}"))?;

    let base = get_base_path().ok_or("No DITCodeAgent base path")?;
    let canonical_tmp = PathBuf::from(&base)
        .join("tmp")
        .canonicalize()
        .map_err(|e| format!("Cannot resolve DITCodeAgent tmp directory: {e}"))?;

    if !path.starts_with(&canonical_tmp) {
        return Err("Path is outside DITCodeAgent data directory".to_string());
    }

    Ok(path)
}

/// Validate that `session_path` is a real file inside `~/.ditcodeagent/tmp/`
fn validate_session_path(session_path: &str) -> Result<PathBuf, String> {
    let path = validate_ditcodeagent_path(session_path)?;
...
fn read_project_root(project_dir: &Path) -> Option<String> {
    ...
    // Reject symlinked .project_root to prevent reading outside ~/.ditcodeagent/tmp
...
mod tests {
    ...
    fn test_convert_user_message_string_content() {
        ...
        assert_eq!(result.provider, Some("ditcodeagent".to_string()));
