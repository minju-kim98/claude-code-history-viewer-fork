...
이 fork는 사내 도구 프로바이더를 지원하기 위해 유지된다. 현재 **DITCodeAgent**와
**Boim**(prod `boim` / dev `boim-dev`) 세 개이며, 모두 회사 내부 도구이므로
upstream(`jhlee0409/claude-code-history-viewer`)에 역기여하지 않는다.

세 프로바이더 모두 Codex와 동일한 rollout JSONL을 쓰기 때문에 `codex` 파서를
재사용하고 **루트 경로로만 서로를 구분**한다. Boim dev는 `BOIM_DEV_HOME`이 설정된
머신에서만 나타난다.

### 운영 규칙

- `src-tauri/src/providers/{ditcodeagent,boim,boim_dev}.rs`,
  `src/i18n/locales/*/common.json`의 `common.provider.{ditcodeagent,boim,boimDev}`,
  `StatsProvider::{DitCodeAgent,Boim,BoimDev}`,
  `ProviderId.{ditcodeagent,boim,boim-dev}` 관련 코드는 **fork 전용**이며
  upstream PR로 보내지 않는다.
- 이들 전용 버그 fix와 enhancement 커밋은 `fix(ditcodeagent):`, `feat(boim):`
  형태로 scope을 명시해 upstream-sync 시 cherry-pick 대상에서 쉽게 식별/배제할 수
  있도록 한다.
